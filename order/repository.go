...
func (r *postgresRepository) PutOrder(ctx context.Context, o Order) (err error) {
	...
	if err != nil {
		return err
				...
				// The defer block ensures that the transaction is either committed or rolled back 
			// based on whether an error occurred. If err is not nil when the function exits, tx.Rollback() is called. Otherwise, tx.Commit() is called
	defer func() {
		if err != nil {
			tx.Rollback()
			return
